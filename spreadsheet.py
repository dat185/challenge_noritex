...
    """
    ...
    try:
        def get_math_operation(func):
            func = func.upper()
            keyRegex = "[A-Z]{1}[0-9]+"
            keyFullRegex = "-"+keyRegex+"|"+keyRegex
            numberRegex = "([0-9]+.[0-9]+|[0-9]+)"
            numberFullRegex = "-"+numberRegex+"|"+numberRegex
            itemRegex = " *("+keyFullRegex+"|"+numberFullRegex+")"
            resultRegex = re.match("^(=(("+itemRegex+" *[+\\-*/]{1}"+itemRegex+")|"+itemRegex+")|"+numberFullRegex+")", func)
            if resultRegex :
                if resultRegex.group()  == func:
                    resultKeyRegex = re.search(keyRegex,func)
                    if resultKeyRegex:
                        value = resultKeyRegex.group()
                        try:
                            matrixValue = get_math_operation(str(m[int(value[1:len(value)])-1][ord(value[0])-65]))
                            return get_math_operation(func.replace(value,str(matrixValue)))
                        except IndexError:
        ...
        for i in range(len(m)):
            for j in range(len(m[0])):
                m[i][j] = get_math_operation(str(m[i][j]))
        return m
